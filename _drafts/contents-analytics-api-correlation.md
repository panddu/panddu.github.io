---
layout: post
title: "유튜브와 블로그, 두 플랫폼 데이터를 몽땅 분석해서 찾아낸 '지표의 상관관계' (feat. API)"
...
<p>
일 평균 사용자가 약 10명 내외인 작은 규모이지만, <b>평균 체류 시간 4.8분, 참여율 38%</b>라는 숫자는 깊이 있는 텍스트 콘텐츠를 읽으러 오는 고품질 오디언스가 안착하고 있음을 명백히 보여줍니다.
</p>
...
#### ① 롱테일(Long-tail) 유입의 원조, 미니 프로젝트
<p>
가장 높은 조회수를 기록한 글은 뜻밖에도 2019년에 끄적여두었던 <b>'스팸체 생성기'</b>였습니다. 특별히 홍보하지 않아도 구글 검색 엔진 등을 타고 꾸준히 외부 트래픽을 몰고 오는 롱테일 역할을 해내고 있었습니다.
</p>
...
<ul>
  ...
  <li><b>블로그를 통한 롱테일 트래픽 확장</b><br>
    유튜브 영상은 시간이 지나면 추천 알고리즘의 수명이 다하지만, 블로그 글은 <b>'스팸체 생성기'</b> 사례처럼 검색 노출을 통해 영구적인 롱테일 트래픽을 만들어냅니다. 따라서 영상에 들어간 기획이나 스크립트 중 유용한 팁들은 반드시 검색 엔진 최적화(SEO)를 고려해 블로그 텍스트로 보존합니다.
  </li>
</ul>

<p>
데이터를 분석하기 전에는 유튜브 조회수 하락에 섭섭해하고 블로그에는 글을 띄엄띄엄 썼었는데, API 데이터를 결합해 보니 두 플랫폼이 어떻게 톱니바퀴처럼 서로를 밀어주고 있는지 명확하게 볼 수 있었습니다. 역시 느낌 대신 데이터에 질문을 던지는 것이 가장 확실한 성장의 열쇠인 것 같습니다.
</p>
