...
Hangle Syllables 은 한글 음절 영역입니다. '가'에서 '힣'까지입니다. (0xAC00 ~ 0xD7A3) 위 초성, 중성, 종성의 순서대로 조합되어 있습니다.
...
예를 들어 봅시다. 첫 글자인 '가'의 경우, 각 자모 배열의 0번째 인덱스(첫 번째)입니다. 따라서 (0 * 21 * 28) + (0 * 28) + 0 = 0입니다. 0xAC00 부터 0번째 뒤에 있는 문자인 것입니다.

그렇다면 0xAC01과 0xAC02는 무엇일까요? 1씩 늘어났으므로 종성의 인덱스가 증가되었음을 알 수 있습니다. 따라서 각각 '각', '갂'이 되겠죠.

그럼 '나'의 경우는? 'ㄴ'은 세 번째 초성(인덱스 2)이며 중성, 종성은 첫 번째(인덱스 0)이므로 (2 * 21 * 28) + (0 * 28) + 0 = 1176 즉, '가'로부터 1176번째 뒤에 있는 값입니다.
...
*참고: 실제 구현 시 나눗셈(`/`) 연산은 소수점 버림 처리(JS의 `Math.floor()`나 Python의 `//` 등)가 필요합니다.*
