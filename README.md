...
- **글 목록 "더보기" 버튼** (홈·글 메뉴·시리즈 페이지 공용, `_includes/post_list.html`) — 전체 글을 다 렌더링해두고 JS로 10개씩 노출하는 방식(`assets/css/main.scss`의 `.post-list__item--more`, `_layouts/default.html`의 로드모어 스크립트). 이미지 없는 텍스트 카드라 글 1개당 gzip ~150바이트 수준 — **글 500개 이상**부터 페이지당 추가 용량이 체감될 만해지므로 그때 가서 진짜 정적 페이지네이션(`jekyll-paginate` — GitHub Pages 화이트리스트 플러그인, 단 시리즈별 필터링은 미지원이라 시리즈까지 하려면 `jekyll-paginate-v2` + GitHub Actions 빌드 전환 필요)을 고민하면 됨. 2026년 기준 43개 글, 연 6~30개 페이스라 당분간 해당 없음.
